...
def display_layer1_weights(network):
    """
    Displays the weights of the first convolutional layer (conv1) of the network.
    ...
    for i, ax in enumerate(axes.flat[:10]):
        ...
        print(f"Filter {i+1} with shape {kernel.shape}:\n{kernel}\n")
...
def apply_filter_to_image(model, img, label):
    """
    Applies each filter from the first convolutional layer (conv1) of the model
    to a single input image and displays the filtered results.

    Args:
        model (nn.Module): The trained neural network model.
        img (Tensor): A single MNIST image tensor of shape (1, 28, 28).
        label (int): The label of the MNIST image for display.
    """
...
def main(argv):
    ...
    if(len(argv) < 2):
        print("Usage: python mnist_network_examination.py <model_name>")
        return
    model_name = argv[1]
